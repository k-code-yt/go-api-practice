...
func (currV Vector2D) DistanceSq(newV Vector2D) float64 {
	return (currV.x-newV.x)*(currV.x-newV.x) + (currV.y-newV.y)*(currV.y-newV.y)
}
...
func (currV Vector2D) LenSq() float64 {
	return (currV.x * currV.x) + (currV.y * currV.y)
}
