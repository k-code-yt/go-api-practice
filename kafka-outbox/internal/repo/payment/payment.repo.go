...
func (pr *PaymentRepo) Save(ctx context.Context, p *Payment) (int, error) {
	return reposhared.TxClosure(ctx, pr.repo, func(ctx context.Context, tx *sqlx.Tx) (int, error) {
		...
		event := event.NewEvent(event.EventType_PaymentCreated, strconv.Itoa(paymentID), metadata)
...
func (r *PaymentRepo) insert(ctx context.Context, tx *sqlx.Tx, p *Payment) (int, error) {
	_, err := tx.NamedExecContext(ctx, fmt.Sprintf("INSERT INTO %s (order_number,amount,status,created_at,updated_at) VALUES(:order_number,:amount,:status,:created_at,:updated_at)", r.tableName), p)
	if err != nil {
